	...
	node(n)
{
	...
	ret = pool_init(&pool, MAX(DEFAULT_QUEUELEN, node->in.vectorize), SAMPLE_LEN(node->samplelen), &memory_hugepage);
	if (ret) {
...
void PathSource::read(struct path *p, int i)
{
	...
	recv = node_read(node, read_smps, allocated, &release);
	if (recv == 0)
		goto out2;
	else if (recv < 0)
...
PathDestination::PathDestination(int queuelen)
{
	...
	ret = queue_init(&queue, queuelen, &memory_hugepage);
	if (ret) {
		// @todo: throw exception
	}
}
...
void PathDestination::write()
{
	...
	while (1) {
		...
		debug(LOG_PATH | 15, "Dequeued %u samples from queue of node %s which is part of path %s", allocated, node_name(node), path_name(p));

		release = allocated;

		sent = node_write(node, smps, allocated, &release);
		if (sent < 0)
			error("Failed to sent %u samples to node %s", cnt, node_name(node));
		else if (sent < allocated)
			warn("Partial write to node %s: written=%d, expected=%d", node_name(node), sent, allocated);
...
int Path::init()
{
	...
	ret = pool_init(&pool, MAX(1, destinations.size()) * queuelen, SAMPLE_LEN(samplelen), &memory_hugepage);
	if (ret)
		return ret;
